...
This repository currently holds its off-chain event indexer: a service that watches a configurable Soroban contract, decodes the events it emits, stores them in SQLite, and serves them through a REST API and a small live dashboard. It works against any Soroban contract by id, not just Flux-toile's own — see the worked example below, which points it at a real, live AMM.
